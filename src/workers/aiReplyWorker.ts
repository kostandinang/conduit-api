...
import { GenerateAIReplyJobData } from '../types';
...
export async function registerAIReplyWorker() {
  ...
  await queue.work<GenerateAIReplyJobData>(
    ...
    async (job) => {
      ...
      logger.info(
        { jobId: job.id, leadId: lead_id, channel },
        'Processing generate-ai-reply job'
      );
      ...
      try {
        ...
        logger.error(
          { error, jobId: job.id, leadId: lead_id },
          'generate-ai-reply job failed'
        );
